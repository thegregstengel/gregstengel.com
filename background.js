(function () {
  ...
  const LABELS = {
    ...
    "datadog": "Datadog", "splunk": "Splunk", "python": "Python", "bash": "Bash",
    "firebase": "Firebase", "linux": "Linux", "ubuntu": "Ubuntu",
    "fedora": "Fedora", "windows": "Windows", "wsl": "WSL", "vscode": "VS Code",
    "vim": "Vim", "cursor": "Cursor", "claude-ai": "Claude", "chatgpt": "ChatGPT",
    "git": "Git", "github-dark": "GitHub", "github-actions": "GitHub Actions",
    "android": "Android", "google-play": "Google Play", "truenas": "TrueNAS",
    "plex": "Plex", "tailscale": "Tailscale", "obsidian": "Obsidian",
    "wikidotjs": "Wiki.js", "jekyll": "Jekyll", "discord": "Discord",
  ...
  const MAX_ICONS_MOBILE = 36;  // lighter cap on small screens (battery/CPU)
  const MOBILE_W = 760;         // matches the CSS breakpoint
  ...
  function build() {
    ...
    const cap = w <= MOBILE_W ? MAX_ICONS_MOBILE : MAX_ICONS;
    ...
    for (const [c, r] of cellOrder) {
      ...
      el.setAttribute("data-label", labelFor(slug));
  ...
  // Debounced rebuild on resize so density stays consistent. Width-only:
  // mobile browsers fire resize when the address bar shows/hides (height
  // change), and a full re-randomize there just makes the wall jump.
  ...
  // Pause the animations while the tab is hidden — no point burning CPU on a
  // background nobody is looking at.
  document.addEventListener("visibilitychange", () => {
    // Clear the inline value when visible (rather than forcing "running") so
    // the CSS hover-to-pause rule isn't shadowed by an inline override.
    const state = document.hidden ? "paused" : "";
    for (const icon of root.children) {
